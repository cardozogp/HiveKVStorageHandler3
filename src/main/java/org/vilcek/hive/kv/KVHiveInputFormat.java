...
public class KVHiveInputFormat extends HiveInputFormat<LongWritable, MapWritable> {
    ...
    private static final Logger log = Logger.getLogger(KVHiveInputFormat.class);
    ...
    public InputSplit[] getSplits(JobConf conf, int numSplits) throws IOException {
        ...
        String kvHostPort = conf.get("kv.host.port");
        Pattern pattern = Pattern.compile(",");
        kvHelperHosts = pattern.split(kvHostPort);
        kvStoreName = conf.get("kv.name");

		kvStoreSecurityFile = conf.get("kv.security");
        ...
        for (int i = 1; i <= nParts; i++) {
            ...
            for (RepNode rn : repNodes) {
                ...
                try {
                    ...
                    log.error("Ping failed for " + rn.getResourceId(), re);
                    re.printStackTrace();
                    statuses.put(rn.getResourceId(), null);
                } catch (NotBoundException e) {
                    log.error("No RMI service for RN: " + rn.getResourceId(), e);
                }
            ...
            if (!isEmpty(parentKeyValue)) {
                ...
                parentKeyValue = conf.get("kv.parent.key");
                if (!isEmpty(parentKeyValue)) {
                    parentKey = Key.fromString(parentKeyValue);
                }
            }
